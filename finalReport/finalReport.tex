...
The algorithm (\Cref{mg:genPoints}) takes 3 parameters, number of points in the shape, an origin point and the range new points.
...
For this reason, the algorithm used is not strictly a Graham Scan but rather heavily based on the first stage, as shown in (\Cref{mg:sortPoints}).
...
The testing results of this section are illustrated in \Cref{PE:mg:Combined}
...
The results of testing for baseline and area are shown in \Cref{PE:p:BaselineAreaRT}, Number of holes and pathing method used in \Cref{PE:p:HolePathRT}
...
This test shows how the number of holes in a field affect the overal run time, this test was conducted on an 80 x 80 square field with a varying number of holes.
...
All measurements of memory usage over the course of testing can be seen in \Cref{PE:p:Mem}
