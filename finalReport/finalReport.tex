...
Headlands are also generated around obstacles to allow for suitable turning area around the obstacles if needed, this servers as an area to do a "U-turn", effictively the same as reaching the end of the field.
...
Both of these have pros and cons, for example lower total length offers better time effiency assuming a near constant travel speed while lower swath count gives reduced turning movements which can be better if turning requires a slower speed or other complexities.
...
These can be sorted in a number of a ways:

\begin{itemize}
	\item{Shortest route to cover all swaths, order depends on field}
	\item{Boustrophen order, a zigzag like pattern covering swaths in order (1,2,3...)}
	\item{Snake order, Similar to Boustrophen but skipping 1 swath each time (1,3,5...)}
