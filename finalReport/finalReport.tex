...
Upon testing the data with theses annotations, it produced faulty results including both true negative and false positives ~\Crefrange{am:AGDanish}{am:ohGCDanish}.
...
The model's performance also varied greatly depending on the shade of green in the area. For instance the Danish image above scores well as it shares lots of chaerectistics with the training data but for a golf course in England with darker greens it scores poorly in comparison ~\Cref{am:AEEnglishCourseResult}.

The fix for this was more diverse training data, the model must understand that different shades can mean the same class is needed.

\paragraph{Training} \

Tracks stats for each class(pixel area, count) > sample weights - boosts rare classes
Tracks per-class accuracy to monitor how well rare classes are being learne

transforms - horizontal flipping, colour jitter and rotation
...
reduces well classified examples to focus on harder cases
...
Tracks loss and accuracy per class
