...
There are several algorithmic methods that try to achieve this task from several different librarys.

Scikit-image's Canny edge detection implementation follows the classic Canny algorithm developed by John F. Canny in 1986. The algorithm works through several steps to detect edges in images:
...
Computes the gradient magnitude (strength of edge)
Computes the gradient direction
...
In scikit-image, the function skimage.feature.canny() implements this algorithm with parameters for:

sigma: The standard deviation of the Gaussian filter
low_threshold and high_threshold: The thresholds for edge classification
mask: Optional mask to limit the region where edges are detected
