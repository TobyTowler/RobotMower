...
def create_green_cells_with_holes(json_file_path, scale=0.1):
    """Create fairway cells with holes using addRing for overlapping obstacles"""
    with open(json_file_path, "r") as f:
        data = json.load(f)

    fairways = [d for d in data["detections"] if d["class"] == "green"]
    obstacles = [d for d in data["detections"] if d["class"] in ["bunker", "water"]]
...
def create_obstacle_cells(
    json_file_path, obstacle_classes=["bunker", "water"], scale=0.1
):
    with open(json_file_path, "r") as f:
        data = json.load(f)
