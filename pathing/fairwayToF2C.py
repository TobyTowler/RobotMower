...
def create_fairway_cells_with_holes(json_file_path, scale=0.1):
    ...
    fairways = [d for d in data["detections"] if d["class"] == "fairway"]
    obstacles = [
        d for d in data["detections"] if d["class"] in ["bunker", "green", "water"]
    ]
...
def create_obstacle_cells(
    json_file_path, obstacle_classes=["bunker", "green", "water"], scale=0.1
):
    with open(json_file_path, "r") as f:
        data = json.load(f)
...
def genFairwayPath(json_file):
    cells = create_fairway_cells_with_holes(json_file, scale=1)
...
if __name__ == "__main__":
    ...
    route = genFairwayPath(json_file)
