...
def main():
    ...
    validation_frequency = 2  # Validate every N epochs to speed up training
    ...
    # NEW: Create weighted sampler for training set to address class imbalance
    train_sampler = WeightedRandomSampler(
        weights=[dataset.sample_weights[i] for i in train_indices],
        num_samples=len(train_indices),
        replacement=True,
    ...
    # Create data loaders - now with weighted sampler for training
    train_loader = DataLoader(
        train_dataset,
        batch_size=2,
        sampler=train_sampler,  # NEW: Use weighted sampler instead of shuffle
    ...
    # NEW: Use a better optimizer - Adam instead of SGD
    optimizer = torch.optim.Adam(params, lr=0.0001)

    # NEW: Use a more appropriate learning rate scheduler
    ...
    early_stopping_patience = 15  # Reduced patience for faster feedback
    best_loss = float("inf")
    best_epoch = 0
    no_improvement_count = 0
