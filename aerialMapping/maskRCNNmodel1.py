...
def train_one_epoch(model, optimizer, data_loader, device, class_weights=None):
    ...
    for images, targets in data_loader:
        ...
        # Forward pass - standard Mask R-CNN forward
        loss_dict = model(images, targets)

        # Apply class weights to classification loss if provided
        if class_weights is not None:
            # Weight the classification loss
            if "loss_classifier" in loss_dict:
                loss_dict["loss_classifier"] *= 1.5  # Emphasize classification loss
...
def main():
    ...
    for epoch in range(max_epochs):
        ...
        loss = train_one_epoch(model, optimizer, train_loader, device, class_weights)
        losses.append(loss)
